...
const testSuperAdminEmail = "receipt-reject-conflict-test-admin@example.com";
const testSiteName = "Receipt Reject Conflict Test Site";
const testPaymentBatchTitle = "Receipt Reject Conflict Test Payment";
...
describe("Payment Receipt Reject Conflict", () => {
  beforeAll(async () => {
    await cleanupTestData();

    const superAdmin = await prisma.user.create({
      data: {
        fullName: "Receipt Reject Conflict Test Admin",
        email: testSuperAdminEmail,
        phone: null,
    ...
    const paymentBatch = await prisma.paymentBatch.create({
      data: {
        title: testPaymentBatchTitle,
        description: "Receipt reject conflict test payment.",
        totalAmountKurus: 21000,
        scopeType: "SITE",
    ...
    const approvedReceipt = await prisma.paymentReceipt.create({
      data: {
        ...
        originalFileName: "reject-conflict-approved-receipt.pdf",
        storedFileName: "reject-conflict-approved-receipt.pdf",
    ...
    const rejectedReceipt = await prisma.paymentReceipt.create({
      data: {
        paymentAllocationId: allocation.id,
        uploadedByUserId: testSuperAdminId,
        originalFileName: "reject-conflict-rejected-receipt.pdf",
        storedFileName: "reject-conflict-rejected-receipt.pdf",
        mimeType: "application/pdf",
        sizeBytes: 100,
        status: "REJECTED",
        reviewedAt: new Date(),
  ...
  it("PATCH /api/payment-receipts/:receiptId/reject should reject approved receipts", async () => {
    const auth = await buildAuthenticatedRequest();

    const response = await request(app)
      .patch(`/api/payment-receipts/${approvedReceiptId}/reject`)
      .set("x-csrf-token", auth.csrfToken)
      .set("Cookie", auth.cookies)
      .send({
        reviewNote: "Onaylanmış dekont red test notu",
      });
  ...
  it("PATCH /api/payment-receipts/:receiptId/reject should reject already rejected receipts", async () => {
    const auth = await buildAuthenticatedRequest();

    const response = await request(app)
      .patch(`/api/payment-receipts/${rejectedReceiptId}/reject`)
      .set("x-csrf-token", auth.csrfToken)
      .set("Cookie", auth.cookies)
      .send({
        reviewNote: "Zaten reddedilmiş dekont test notu",
      });
